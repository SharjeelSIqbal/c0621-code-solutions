...
app.post('/api/grades', (req, res, next) => {
  ...
  if (!name || !course || !score) {
    throw new ClientError(400, 'Name, course, and score must be included');
  }
  if (!Number.isInteger(score) || score < 0 || score > 100) {
    throw new ClientError(400, 'score must be a positve number at or below 100');
  }
  const sql = `
...
app.put('/api/grades/:gradeId', (req, res, next) => {
  ...
  db.query(sql, params)
    .then(result => {
      ...
      if (!updatedGrade) {
        throw new ClientError(404, 'cannot find with gradeId');
      } else {
